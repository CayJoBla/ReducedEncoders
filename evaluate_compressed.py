...
def evaluation(model=None, dataset="glue", task_name=None, tokenizer=None, revision="main", output_dir=None, 
    ...
    ## Load config
    model_name = "cayjobla/all-mpnet-base-v2-compressed" if model is None else model
    config = MPNetReducedConfig.from_pretrained(model_name, revision=revision, num_labels=num_labels)

# train_compressed_mistral_accelerate.py

import torch
from torch import nn, optim
from sentence_transformers import SentenceTransformer
from datasets import load_dataset
import json
from torch.utils.data import DataLoader
from accelerate import Accelerator
from accelerate.utils import set_seed
import os
import random
import numpy as np
import torch

from reduced_encoders.modeling_reduced_new import MultiResize
from reduced_encoders.modeling_utils import UnsupervisedCosineSimilarityLoss

# -------------------------------- Configuration -------------------------------

SEED = 916

# Base model config
BASE_MODEL = "tomaarsen/NeoBERT-gooaq-8e-05"
MAX_SEQ_LENGTH = 4096

# Reduction config
DROPOUT = 0.1
SIZES = [768,512,256,128,64,48]
ACT_FN = nn.SiLU()

# Dataset config
DATASET_PATH = "sentence-transformers/gooaq"
SPLIT = "train"
EVAL_SIZE = 5000

# Training config
RUN_NAME = "neobert-reduced-no-reconstruction"
LR = float(os.getenv("TRAIN_LR"))
BATCH_SIZE = 6
NUM_EPOCHS = 1
OUTPUT_DIR = "/home/cayjobla/ReducedEncoders/NeoBERT-reduced-no-reconstruction"
LOGGING_STEPS = 2500
CLEAR_CUDA_CACHE_STEPS = 1000
EVAL_STEPS = 50000
SAVE_STEPS = 50000

# ------------------------------ Initialization --------------------------------

# Set seeds for reproducibility
random.seed(SEED)
np.random.seed(SEED)
torch.manual_seed(SEED)
torch.cuda.manual_seed_all(SEED)
set_seed(SEED)

# Initialize the accelerator
accelerator = Accelerator(log_with="wandb")

# ---------------------------- Model Initialization ----------------------------

# Load the model
with accelerator.main_process_first():  # Allow secondary processes to load from cache
    model = SentenceTransformer(BASE_MODEL, trust_remote_code=True)
    model.max_seq_length = MAX_SEQ_LENGTH

# Prepare the model for inference (no training on full sentence transformer)
for param in model.parameters():
    param.requires_grad = False
model.eval()

accelerator.wait_for_everyone()
accelerator.print(f"Loaded model: {BASE_MODEL}")

# Initialize the reduction and reconstruction layers
reduce = MultiResize(
    sizes=SIZES, 
    bias=True, 
    activation_function=ACT_FN,
    dropout=DROPOUT
)

accelerator.wait_for_everyone()
accelerator.print(f"Initialized reduction and expansion layers with sizes: {SIZES}")

# -------------------------- Dataset Initialization ----------------------------

# Load and split the dataset into train and eval
dataset = load_dataset(DATASET_PATH, split=SPLIT)
dataset = dataset.train_test_split(test_size=EVAL_SIZE)

# Initialize the dataloaders
train_dataloader = DataLoader(dataset["train"], batch_size=BATCH_SIZE, shuffle=True)
eval_dataloader = DataLoader(dataset["test"], batch_size=BATCH_SIZE, shuffle=False)

accelerator.wait_for_everyone()
accelerator.print(f"Loaded dataset: {DATASET_PATH}, split: {SPLIT}, eval size: {EVAL_SIZE}")

# --------------------------- Loss Initialization ------------------------------

# Initialize the loss function
loss_fn = UnsupervisedCosineSimilarityLoss(     # Defaults
    similarity_fn=nn.CosineSimilarity(dim=-1),
    loss_fn=nn.MSELoss(reduction="mean")
)
accelerator.wait_for_everyone()
accelerator.print(f"Initialized UnsupervisedCosineSimilarityLoss")

# ------------------------- Optimizer Initialization ---------------------------

# Initialize the optimizer
optimizer = optim.AdamW(reduce.parameters(), lr=LR)

# Initialize lr scheduler
num_training_steps = len(train_dataloader) * NUM_EPOCHS
lambda_lr = lambda step: max(0, (num_training_steps - step) / num_training_steps)
lr_scheduler = optim.lr_scheduler.LambdaLR(optimizer, lambda_lr)

accelerator.wait_for_everyone()
accelerator.print(f"Initialized optimizer and LR scheduler")

# ---------------------------- Training Prep -----------------------------------

def forward(batch):
    with torch.no_grad():
        full_embeddings = model.encode(
            batch["question"] + batch["answer"],
            convert_to_tensor=True,
            show_progress_bar=False
        ).detach()

    reduced_embeddings = reduce(full_embeddings)

    loss = loss_fn(reduced_embeddings, full_embeddings)
    del full_embeddings, reduced_embeddings

    return loss

def log():
    epoch_frac = global_step / len(train_dataloader)
    accelerator.print(f"Epoch: {epoch_frac}, Step: {global_step}, Train Loss: {running_loss / running_count}")
    accelerator.log({
        "train/epoch": epoch_frac,
        "train/global_step": global_step,
        "train/loss": running_loss / running_count,
        "train/cosine_sim_loss": running_loss / running_count,
        "train/learning_rate": optimizer.param_groups[0]["lr"],
    })

def evaluate():
    accelerator.print(f"Running evaluation (step {global_step})...")
    reduce.eval()

    eval_loss = []
    for eval_batch in eval_dataloader:
        with torch.no_grad():
            loss = forward(eval_batch)
            eval_loss.append(loss.detach())

    eval_loss = accelerator.gather(torch.stack(eval_loss)).mean().item()

    epoch_frac = global_step / len(train_dataloader)
    accelerator.print(f"Epoch: {epoch_frac}, Step: {global_step}, Eval Loss: {eval_loss}")
    accelerator.log({
        "train/epoch": epoch_frac,
        "train/global_step": global_step,
        "eval/loss": eval_loss,
        "eval/cosine_sim_loss": eval_loss,
    })

    reduce.train()

def save(checkpoint=None):
    output_dir = OUTPUT_DIR
    if checkpoint is not None:
        output_dir = os.path.join(output_dir, f"checkpoint-{checkpoint}")
    os.makedirs(output_dir, exist_ok=True)

    accelerator.wait_for_everyone()
    reduce_unwrapped = accelerator.unwrap_model(reduce)
    accelerator.save(reduce_unwrapped.state_dict(), os.path.join(output_dir, "reduce.pth"))

# Initialize the accelerator
model, reduce, optimizer, lr_scheduler, train_dataloader, eval_dataloader = accelerator.prepare(
    model, reduce, optimizer, lr_scheduler, train_dataloader, eval_dataloader
)
accelerator.init_trackers(
    project_name="reduced-encoders",
    config={
        "dropout": DROPOUT, 
        "sizes": SIZES, 
        "activation_function": ACT_FN.__class__.__name__,
        "learning_rate": LR,
        "batch_size": BATCH_SIZE,
        "do_reconstruction": False,
    },
    init_kwargs={"wandb": {"name": RUN_NAME}}
)
accelerator.wait_for_everyone()
accelerator.print(f"Initialized wandb tracking with run name: {RUN_NAME}")

# ---------------------------- Training Loop -----------------------------------

reduce.train()

accelerator.wait_for_everyone()
accelerator.print("Starting training loop...")

running_loss = 0
running_count = 0

global_step = 0
for epoch in range(NUM_EPOCHS):
    # On epoch start
    evaluate()
    save(f"epoch-{epoch}")

    for batch in train_dataloader:
        # Forward pass
        loss = forward(batch)

        # Backward pass
        optimizer.zero_grad()
        accelerator.backward(loss)
        optimizer.step()
        lr_scheduler.step()    # Update learning rate

        # Log running loss
        running_loss += loss.item()
        running_count += 1

        # Clear cuda cache
        if global_step % CLEAR_CUDA_CACHE_STEPS == 0:
            torch.cuda.empty_cache()

        # Training logs
        if global_step % LOGGING_STEPS == 0:
            log()

        global_step += 1    # Avoid rerunning eval/save on first step

        # Evaluation
        if global_step % EVAL_STEPS == 0:
            accelerator.wait_for_everyone()
            evaluate()

        # Save
        if global_step % SAVE_STEPS == 0:
            accelerator.wait_for_everyone()
            save(f"step-{global_step}")

# Final evaluation and save
accelerator.wait_for_everyone()
evaluate()
save()
